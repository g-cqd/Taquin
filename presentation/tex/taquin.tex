...
\paragraph{}{De plus, deux heuristiques intéressantes à combiner étaient la distance de Mahnattan avec une pondération de 1 ainsi que le taux de désordre.En effet, ces deux heuristiques optimales 10 fois sur 10 tentatives mais possédant un temps moyen et un nombre d'espace moyen très élevé séparément  se révèlent être des alliées redoutables lorsqu'elles sont mises en commun. En effet, leur écart moyen à la solution optimal est minime : 0,6 et leur taux d'optimalité acceptable :$\frac{7}{10}$. }
...
\paragraph{}{Pour finir, nous avons tenté de combiner H2 ainsi que H4, les deux heuristiques qui séparées présentent les temps moyen ainsi que le nombre moyen d'états créés le plus faible.  Toutefois nous pouvons constater que mises ensemble ces dernières présentent des performances inférieures et un taux d'optimalité équivalent. }
\end{document}