...
class generationTaquin:
	...
	def generation(self):
		...
		while(grilleSolvable == False):
			...
			listeFin = []
			i = 1
			while(i!=(self.largeur()*self.largeur())):
				liste_init.append(i)
				print(liste_init)
				i = i + 1
			...
			while(permute !=False):
				...
				while(j!=len(liste_init)-1):
					if(liste_init[j]>liste_init[j+1]):
						...
						permute = True
					j = j+1
			if((self.largeur()%2==1)and(nombrePermutation%2==0)):
				grilleSolvable = True
			if((self.largeur()%2==0)and((self.largeur()-ceil(listeFin.index('X')/4))%2==0)and(nombrePermutation%2==1)):
				grilleSolvable = True
			if((self.largeur()%2==0)and((self.largeur()-ceil(listeFin.index('X')/4))%2==1)and(nombrePermutation%2==0)):
				grilleSolvable = True
		return listeFin
